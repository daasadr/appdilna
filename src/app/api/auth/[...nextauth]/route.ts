...
async function refreshAccessToken(token: JWT): Promise<JWT> {
  try {
    console.log(
      '[refreshAccessToken] Používám refreshToken:',
      token.refreshToken
    )
    const payload = {
    ...
    if (!response.ok) {
      ...
      if (
        errorData?.errors?.[0]?.message?.includes('Invalid user credentials')
      ) {
        console.error(
          '[refreshAccessToken] Invalid credentials detected, forcing logout'
        )
        return {
          ...token,
    ...
    console.log(
      '[refreshAccessToken] newTokens.refresh_token:',
      newTokens.refresh_token,
      'old token.refreshToken:',
      token.refreshToken
    )
    ...
    if (
      ...
      newTokens.refresh_token.trim() === ''
    ) {
      console.error(
        '[refreshAccessToken] ERROR: Directus nevrátil platný refresh token! Uživatel bude odhlášen.'
      )
      return {
        ...token,
        error: 'RefreshAccessTokenError',
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        console.log(
          '[Directus authorize] /auth/login response:',
          rawLoginResponse
        )
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      const userRefreshToken =
        user && 'refreshToken' in user ? (user as any).refreshToken : undefined
      ...
      if (user && account) {
        if (account.provider === 'credentials') {
          ...
          token.refreshToken =
            (user as AppUser).refreshToken ?? token.refreshToken
          token.id = user.id
    ...
    async session({ session, token }: { session: Session; token: JWT }) {
      ...
      if (token) {
        ...
        if (token.error === 'RefreshAccessTokenError' || !token.accessToken) {
          console.log(
            'Session error detected or missing access token, user will be logged out'
          )
