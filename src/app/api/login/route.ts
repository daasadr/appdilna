...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isGoogle) {
      ...
      if (users.length === 0) {
        ...
        if (!createRes.ok) {
          ...
          return NextResponse.json(
            { message: 'Nepodařilo se vytvořit uživatele v Directusu.' },
            { status: 500 }
          )
        }
      ...
      const tokenResponse = await fetch(
        `${process.env.DIRECTUS_URL}/auth/login/${user.id}/token`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${process.env.DIRECTUS_ADMIN_TOKEN}`,
      ...
      if (!tokenResponse.ok) {
        ...
        return NextResponse.json(
          { message: 'Nepodařilo se vytvořit token pro uživatele' },
          { status: 500 }
        )
      }
      ...
      if (!data || !data.access_token) {
        console.error(
          'Nepodařilo se získat access_token z Directusu',
          loginResponse
        )
      ...
      const userInfo = await userDirectus.request(
        readMe({
    ...
    try {
      ...
      const tokenResponse = await fetch(
        `${process.env.DIRECTUS_URL}/auth/login`,
        {
          method: 'POST',
    ...
    const errorMessage =
      error.message || 'Došlo k neznámé chybě při přihlašování.'
