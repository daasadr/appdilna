  ...
  try {
    ...
    if (fieldsResponse.ok) {
      ...
      fields.data.forEach(field => {
        console.log(
          `  - ${field.field}: ${field.type}${field.schema?.is_nullable ? ' (nullable)' : ''}`
        )
      })
      ...
      const missingFields = requiredFields.filter(
        field => !fields.data.find(f => f.field === field)
      )
    ...
    if (rolesResponse.ok) {
      ...
      if (registeredRole) {
        ...
        const permissionsResponse = await fetch(
          `${DIRECTUS_URL}/permissions?filter[role][_eq]=${registeredRole.id}&filter[collection][_eq]=apps`,
          {
            headers: {
              Authorization: `Bearer ${ADMIN_TOKEN}`,
              'Content-Type': 'application/json',
        ...
        if (permissionsResponse.ok) {
          ...
          console.log(
            `📋 Nalezeno ${permissions.data.length} oprávnění pro kolekci apps`
          )

          permissions.data.forEach(perm => {
            console.log(
              `  - ${perm.action}: ${perm.permissions ? 'povoleno' : 'zakázáno'}`
            )
          })
        }
      } else {
