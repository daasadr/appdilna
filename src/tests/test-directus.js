...
async function testBasicConnection() {
  try {
    ...
    if (response.ok) {
      ...
      console.error('❌ Directus server not accessible:', response.status, response.statusText);
    }
  } catch (error) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (rolesResponse.ok) {
        ...
        console.log('Available roles:', rolesData.data?.map(r => ({ id: r.id, name: r.name })) || []);
      }
    } else {
      console.error('❌ Admin token is invalid:', response.status, response.statusText);
    }
  } catch (error) {
